...
void triangulation(CtrlStruct *cvs)
{
	...
	pos_tri->x = first_order_filter(old_pos_tri_x, pos_tri->x, 0.4, delta_t);
	pos_tri->y = first_order_filter(old_pos_tri_y, pos_tri->y, 0.4, delta_t);
	pos_tri->theta = first_order_filter(old_pos_tri_theta, pos_tri->theta, 0.4, delta_t);
	
	pos_tri->last_t = inputs->t;
	
	// ----- triangulation computation end ----- //
        
	set_output(pos_tri->x, "x_tri_[m]");
	
	set_plot(pos_tri->x, "x_tri");
	//set_plot(pos_tri->y, "y_tri");
    //set_plot(pos_tri->theta, "theta_tri");
