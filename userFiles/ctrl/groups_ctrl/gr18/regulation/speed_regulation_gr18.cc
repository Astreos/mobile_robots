...
void speed_regulation(CtrlStruct *cvs, double r_sp_ref, double l_sp_ref)
{
	...
	// wheel commands
	outputs->wheel_commands[0] = Kp * (r_sp_ref - r_sp) + Ki * limit_range(sp_reg->int_error_r, -fabs(r_sp_ref - r_sp), fabs(r_sp_ref - r_sp));
	outputs->wheel_commands[1] = Kp * (l_sp_ref - l_sp) + Ki * limit_range(sp_reg->int_error_l, -fabs(r_sp_ref - r_sp), fabs(r_sp_ref - r_sp));
