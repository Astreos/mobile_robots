...
int turn(CtrlStruct *cvs, double theta_ref, int sens)
{
	...
	int Kp = 10; //Kc = 90
        ...
        speed_regulation(cvs, Kp * (theta_ref - rob_pos->theta) * dir, -Kp * (theta_ref - rob_pos->theta) * dir);
...
int run_x(CtrlStruct *cvs, double x_ref)
{
	...
	int Kp = 10; //Kc = 90
        ...
        speed_regulation(cvs, Kp * (x_ref - rob_pos->x), Kp * (x_ref - rob_pos->x));

	// ----- Wheels regulation computation end ----- //
        
        if (fabs(x_ref - rob_pos->x) < 0.01)
        {
            return 1;
        }
        else
...
int run_y(CtrlStruct *cvs, double y_ref)
{
	...
	int Kp = 10; //Kc = 90
        ...
        speed_regulation(cvs, -Kp * (y_ref - rob_pos->y), -Kp * (y_ref - rob_pos->y));
        
	// ----- Wheels regulation computation end ----- //
        
        if (fabs(y_ref - rob_pos->y) < 0.01)
        {
            return 1;
        }
        else
